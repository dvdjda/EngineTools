...
# ---------- Excel ----------
def build_excel(engine, values, result, path):
...
# ---------- PDF ----------
def build_pdf(engine, values, result, path, chart_png):
...
# ---------- PPTX ----------
def build_pptx(engine, values, result, path, chart_png):
    ...
    txt(0.55, 0.35, 12.2, 0.7, [[(engine.name, 24, True, navy)]])
    txt(0.55, 1.05, 12.2, 0.4, [[("Nexa Block v1  -  screening report", 13, False, grey)]])
    ...
    card(0.55, 1.7, 3.7, 2.5, "Design point",
    ...
    txt(0.55, 7.05, 12.2, 0.35,
        [[("Nexa Power Investments LLC - Confidential   |   screening report, not for certification", 9, False, grey)]])
    prs.save(path)
    return path
