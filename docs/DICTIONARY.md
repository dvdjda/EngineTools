...
**Control modes** — Two mode switches on the v2 GT engine: `operating_mode` (Island / Grid-tied) and `gt_power_mode` (Auto / Manual), plus the `external_load_kW` scalar (functionally tied to operating_mode). The old `steam_split_mode` switch is **removed** — there is no steam splitter, so `libr_frac` is constant 1.0.
...
**MED bypass (`med_bypass_frac`)** — A manual 3-way valve fraction routing part of the LiBr heat-rejection loop *around* MED. MED captures `(1 − med_bypass_frac)` of the loop heat; the bypassed share stays hot and goes to the radiator. Default 0 (all rejection to MED; radiator idles).
