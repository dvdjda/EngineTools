...
**Control modes** — Three mode switches on the v2 GT engines: `operating_mode` (Island / Grid-tied), `gt_power_mode` (Auto / Manual), and `med_bypass_mode` (Manual / Auto — see **MED bypass**), plus the `external_load_kW` scalar (functionally tied to operating_mode). The old `steam_split_mode` switch is **removed** — there is no steam splitter, so `libr_frac` is constant 1.0.
