...
**COP — Coefficient of Performance** — For the LiBr chiller, `COP = Q_cool / Q_gen`. In the GT-system nexablock block: single-effect range 0.5–0.85 (audit check P4 enforces (0.5, 1.3)), default 0.7; the `DoubleEffectLiBrChiller` runs ~1.2. The standalone **LiBr-H₂O absorption chiller** tool is calibrated to the BROAD XII OEM nameplate — single-effect 0.76, double-effect 1.42 at the rated point (chilled 7/14 °C, cooling 30/37 °C) — capped at nameplate and degrading below the rated drive (e.g. a 98 °C source on the double-effect cycle reads ~0.85 and the diagram flags "too cool for double-effect").
...
**`gt_system_v2`** — The v2 GT system engine. Drop-in replacement for the v1 trusted GT tool, validated within ±2 % across 14 reference KPIs (`tests/test_gt_system.py`). **Currently `draft`** (demoted from trusted, pending re-verification).

**`gt_system_v2_loadsweep`** — A screening variant of `gt_system_v2` that runs a 50–100 % GT-load sweep in the chart slot instead of the SVG flowsheet. **Currently `draft`.**

**`gt_system_v2_de`** — The v2 double-effect GT system engine. Identical composition to `gt_system_v2` but the LiBr chiller is a `DoubleEffectLiBrChiller` (COP ~1.2 vs ~0.7, so ~1.7× more cooling and more MED water from the same HRSG steam, at the cost of needing higher-grade steam — the T11 audit check flags too-cool HTG steam). Adds the per-pump/fan plant-aux sizing rows and the auto MED-bypass cascade. **Currently `draft`** (demoted from trusted, pending re-verification).

**`gt_system_v2_de_backup`** — The v2 **trusted** double-effect engine plus a Tier-3 standby architecture: a **DieselGenset** that takes over as prime mover on a **GT-failure** switch (its ~480 °C exhaust still drives the double-effect chiller; the tower covers the rest); a **wet cooling tower** (`CoolingTowerLoop`) replacing the dry radiator (normal reject + top-up, and full GPU cooling on a **LiBr-failure** switch, scaled to wet-bulb), whose fan rides the total tower duty as the **"Cooling-tower fan"** plant-aux line; diesel + fresh-water storage; and **resilience KPIs** — diesel fuel autonomy vs a backup-hours target, fresh-water buffer (MED banks it, tower make-up draws it), **UPS** electrical ride-through and **thermal-accumulator** cooling ride-through (do they cover the diesel/tower start?). **Both failures are modelled in the solve**: a GT trip swaps in the diesel, and a LiBr trip makes the chiller report **0 cooling** while the cooling tower carries the full GPU heat (the E5 energy check closes at 0 = 0). The resilience KPIs (screening-basis) size the storage/ride-through and flag whether the tower can cool the dielectric directly at the chosen wet-bulb. Computed in `simulators/gt_system/backup.py`.
...
**LiBr-H₂O absorption chiller (standalone tool)** — The `libr_chiller` engine (`nexa_toolkit/engines/libr_chiller_engine.py` + `nexa_toolkit/engine/libr_chiller.py`), distinct from the GT-system's embedded `LiBrChiller` block. A single **Cycle** switch selects **single-effect** or **double-effect**; the cycle is calibrated to the **BROAD XII Non-Electric Chiller** OEM datasheet (single COP 0.76 / ~43 % LiBr; double COP 1.42 / ~54 % LiBr at the rated condition). The double-effect mode models the **HTG + LTG** staging with internal heat recovery and three pressure levels (HTG high · LTG = condenser mid · evaporator = absorber low). An optional **make-up gas burner** (`burner_on` + `q_source_avail_kw`) covers any generator-heat shortfall and reports natural-gas use (Nm³/h) on the OEM 10 kWh/Nm³ basis — validated against the direct-fired DFA gas table. Its chart is a GT-style **SVG process flow diagram** (`reporting/libr_pfd.py`) with live stream tags + a duty/crystallisation footer band, and it exposes `study_hooks` (sensitivity / sweep / scenarios) including a waste-heat-cap sweep for fuel-vs-cooling studies. **Currently `draft`.** OEM-anchored COP and nominal concentration are `verified`-basis; off-design values, duties, weak/strong split, burner fuel and crystallisation are `screening`.
...
**Plant aux** — The itemised, IT/flow-driven plant electrical model in `simulators/gt_system/plant_loads.py`. Each **pump** = `Q·ΔP/η` (dielectric coolant, LiBr internal solution, cooling-loop, HRSG feed-water/BFP, seawater intake, MED feed, brine, distillate, condensate). The **dry-cooler fan** rides a VSD: `fan = fan_rated_frac · Q_cond · utilisation³` (cube law on dry-cooler utilisation). In the Backup engine the dry radiator is a **wet cooling tower**, so this line becomes the **"Cooling-tower fan"** = `0.02 · (loop-trim reject + GPU cooling carried by the tower)` — non-zero in a failure mode where the tower tops up or fully carries the GPU heat (the controller mirrors the same term so the bus still closes). **HVAC** = `n_containers · area · U · (T_ambient − T_inside)` with `n_containers = containers_per_MW · IT_MW`; **lights** = `lights_frac · HVAC`. All lines appear in the power-balance breakdown. (GT aux is *not* here — it's an internal GT derate; see **GT net power**.) Replaces the old lumped `libr_pump_frac` / `ct_fan_frac` / `bop_frac` fractions and the MED 1.5 kWh/m³ rule.
